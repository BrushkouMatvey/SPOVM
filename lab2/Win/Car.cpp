...
PROCESS_INFORMATION createNewProcess(char * eventID)
{
	...
	char args[30];
	sprintf(args, "child.exe %s", eventID);
...
void addOneProcess(int id) 
{
	...
	closeEventInfo.push_back(CreateEvent(NULL, FALSE, FALSE, EventID));//2-автоматический сброс, 3-начальное состояние FALSE,3-имя объекта

	sprintf(EventID, " %dp", id);
	printEventInfo.push_back(CreateEvent(NULL, TRUE, FALSE, EventID));//2-ручной сброс, 3-начальное состояние FALSE,3-имя объекта
}
